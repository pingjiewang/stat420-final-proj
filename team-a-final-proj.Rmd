...
### 2.0.2 Data cleaning and unused columns removal
...
We have decided to narrow our data to include the last 20 years of used cars starting with the year 2000 which is more useful for us, and also keep the data where offertype = "Anagebot" and seller ="private" and alos removed any unwanted columns within our dataset.
```{r}
#final data format
str(autos)
```
...
### 2.0.3 Setup training and testing data
...
#### 2.2.1.6 (to be removed)- Run box-cox tranformation all for groups with > 300 records
...
#### 2.2.2.0 (new) - Run box-cox tranformation on all training data
...
#### 2.2.2.1a verify regression signifcant with ANOVA test on high p-Value betas(vehicleType and fuelType)
...
```{r,eval=FALSE}
model3_add = lm(price ~ ., data = autos_train)
diagnostics(model3_add)
par(mfrow=c(1,1))
boxcox_input_formula= as.formula ( as.character(model3_add$call[2]) )
out=boxcox(model3_add, plotit = TRUE, lambda = seq(-0.5, 1.0, by = 0.05))
( lambda=out$x[which.max(out$y)] )
model3_start = lm( ((price^lambda-1)/lambda) ~ .^2 , data = autos_train )
n=nrow(autos_train)
model3=step(model3_start,k=log(n))
diagnostics(model3)
summary(model3)
...
train_error = c(
  rmse(autos_train$price, predict(model1_selected, autos_train, type = 'response')),
  rmse(autos_train$price, predict(model2_selected, autos_train, type = 'response'))
)

# calculate all test errors
test_error = c(
  rmse(autos_test$price, predict(model1_selected, autos_test,type = 'response')),
  rmse(autos_test$price, predict(model2_selected, autos_test,type = 'response'))
)
