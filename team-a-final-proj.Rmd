...
par(mfrow=c(1,2))
model_add = lm(price ~ powerPS + kilometer , data = autos_1)
#fitted vs residual
...
#qqplot
qqnorm(resid(model_add), main = "Normal Q-Q Plot", col = "dodgerblue")
...
model_add_cox=lm((price^lambda-1)/lambda ~ powerPS + kilometer , data = autos_1)

par(mfrow=c(1,2))
plot(fitted(model_add_cox), resid(model_add_cox), col = "dodgerblue", 
     xlab = "Fitted", ylab = "Residuals", main = "Fitted versus Residuals")
abline(h = 0, col = "darkorange", lwd = 2)
qqnorm(resid(model_add_cox), main = "Normal Q-Q Plot", col = "dodgerblue")
qqline(resid(model_add_cox), col = "dodgerblue", lwd = 2)

shapiro.test(resid(model_add_cox))
bptest(model_add_cox)
