...
The dataset we are using for this project was found on Kaggle, the well-known Machine Learning Competition website and it is about Used Car listings from eBay – Germany please see link here: https://www.kaggle.com/orgesleka/used-cars-database/home. Data file contains approximately 370,000 observations and 20 variables that are scraped from used-car listing on Ebay-Kleinanzeigen (German). The high quantity and authenticity make this dataset useful for an exploratory analysis. Furthermore, the dataset is very large but what we have done to narrow it down is that we have decided to filter the data based on the year registration variable starting the year 2000 to now so we can build a good model based on the last 20 years in the used auto industry in Germany. Furthermore, we randomly sampled 100,000 records after our data cleaning process, and split half the data in our training dataset and test dataset which resulted in 50,000 records each for our model prediction. Some of the of the variables that are very useful to us where price is used as a response to help build model predication when combined it with other variables but not limited to: Power, Kilometer, VehicleType, YearRegistration as part of our additive and transformation model for this project. 

This project focuses on finding a good model for predicting based on the statistical knowledge that we gained from the class. We will try to determine the relationship between different variables, between the chosen predictor variable and price. Furthermore, our goal is  to build a MLR model for used car price prediction where we use price as the response variable, the predictors will be selected from all remaining continuous numerical variables such as (PowerPs, Kilometer, yearRegistrations) and the other  categorical variables so we can make compare 3 different methods so we can arrive to the best model that will fit our prediction. We will also be using the leftover observation to validate our model performance trough examining our assumptions by using regression diagnostic testing. Other method that we will be using are collinearity to check for issues on model and dummy variables for categorical fields. Finally, additional methods such as: outlier diagnostics, polynomial regression, and transformations will be leveraged when needed to further analyze and prove that our final model is justified.
...
- Keep only data with YearOfRegistration>=2000,  
- Removed unused columns: `r columns_remove`, 
...
str(autos)
```
We need to note that as part of our data cleaning process we had to remove certain aspect tied to the different variables that we are using for example, we decide to remove price < $0 because a car cannot be associated with $0 pricing. We also had to keep only the data associated with Type== "Angebot" and seller=="private because 99% percent of the data is associated with these fields. In addition, we narrowed our dataset to include the last 20 years in the Ebay Germany dataset starting with the year 2000, and we also removed unused/unwanted columns in our clearning process then we Identified both the continuous and factor variables upon the completion of our data cleaning process.
...
```
Based on our observation, we have determined there little to no collineararity issues with our model
...
Removing the high influential points (about 3% of observation) does improve the model a little. But it is still not ideal. We will save the model (model1_selected) as the first candidate model. In the result section, we will compare its performance with other candidate models. 

Because there seems to be a violation of the linearity assumption, we will introduce response and predictor transformation in the next method.
...
To reduce the price variability due to different combinations of factor variable values, we will temporarily isolate a dataset for the analysis phase. Later, we will use the full training dataset in Phase 2 steps. To do this, we will use 'dplyr' package count() to group dataset by all factor variables. 
...
Next, we will break our dataset into groups, we choose group #2 to isolate a dataset by filtering from the main dataset.
...
Using a dataset from Kaggle and using data from the German eBay used car ads, we started by doing significant amounts of data cleaning. We refrained from doing any translations or conversions of data from German language to English due to our limited time for this project. However, we narrowed the size of our data to 100,000 from 370,000, which really helped us to further analyse, inspect, observe and understand the data.

We extracted the continuous variables that are associated with the response variable price. We divided 100,000 records into subsets of training and test dataset, each having 50,000 records.

In *method 1*, we tried fitting an additive model with all variables. Then using BIC backward search, found the model best fitted where predictors chosen were vehicleType, yearOfRegistration, gearbox, powerPS, kilometer, fuelType and notRepairedDamage. This model clearly violated the linearity and constant variance assumption and the normality assumption is also violated as seen from Q-Q plot.

We further removed the influential points and refitted the model. We will use this model for further comparisons.

In *method 2*, we broke the dataset into subgroups by categorizing the variables abtest, vehicleType, gearbox, model, fuelType and brand. For example, abtest can only have either be test or control, and vehicleType can either be Limousine or Kleinwagon. Lastly we picked the groups with more than 300 records in order to find a definitive model for our dataset.

After trying multiple ways like fitting an additive model using only continuos variables, using Box-Cox on a selected group with predictors , applying log transformations and using backward BIC search we finally found a model that might work. It was an interaction model where price was transformed using boxcox transformation and vehicleType, gearbox, powerPS, kilometer, fuelType, notRepairedDamage and yearOfRegistration as predictors.

Results section shows the comparison between additive and interaction models by calculating their test and train RMSEs. Interaction model test and train RMSEs were lower than the additive model showing that interaction model is our preferred model!
